<script lang="ts">
	...
	let {
		...
		active?: 'feed' | 'explore' | 'leaderboard' | 'aistudio' | 'roasts' | 'profile' | 'upload' | 'notifications';
		initials?: string;
	} = $props();
...
<a
	...
	<div
		...
		>D</div
	>
	<span style="font-family:var(--font-mono); font-weight:500; font-size:15px; letter-spacing:-0.01em;"
		>/DEVFOLIO</span
	>
</a>
...
>
	...
	>
		...
		<span
			style="position:absolute; top:1px; right:2px; background:var(--df-red); color:white; font-size:9px; font-weight:700; width:13px; height:13px; border-radius:50%; display:flex; align-items:center; justify-content:center; border:2px solid white;"
			>3</span
		>
	</a>
	<a
...
>
	{#each items as { key, href, label, Icon }}
		<a
			href={href}
			title={label}
			style="width:40px; height:40px; border-radius:8px; display:flex; align-items:center; justify-content:center; background:{key === active
				? railActiveBg
	...
	<a
		href="/profile"
		title="Profile"
		style="width:40px; height:40px; border-radius:8px; display:flex; align-items:center; justify-content:center; background:{active === 'profile'
			? railActiveBg
